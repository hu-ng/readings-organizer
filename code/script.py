...
class ClassMaterialEventHandler(RegexMatchingEventHandler):
    ...
    def get_class_path(self, path):
        ...
        """
        pattern = r"(?P<name>\w\w?[1-9]{3})_(?P<week>1?[1-9])_(?P<session>[1-2])_?.*"
...
if __name__ == "__main__":
    ...
    event_handler = ClassMaterialEventHandler(regexes=['.*\w\w?[1-9]{3}_1?[1-9]_[1-2]_?.*'])
    path = "C:\\Users\Hung Nguyen\Downloads"
    observer = Observer()
